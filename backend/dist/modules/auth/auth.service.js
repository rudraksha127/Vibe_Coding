...
export async function refresh(refreshToken, context) {
    ...
    if (session.revokedAt) {
        await AuthSession.updateMany({ userId: session.userId, revokedAt: { $exists: false } }, { $set: { revokedAt: new Date() } });
...
export async function logout(userId, sessionId) {
    await AuthSession.updateOne({ _id: sessionId, userId, revokedAt: { $exists: false } }, { $set: { revokedAt: new Date() } });
}
...
export async function revokeSession(userId, sessionId) {
    const result = await AuthSession.updateOne({ _id: sessionId, userId, revokedAt: { $exists: false } }, { $set: { revokedAt: new Date() } });
    if (result.matchedCount === 0) {
        throw new AppError(404, "NOT_FOUND", "Session not found");
    }
}
async function issueTokenPair(userId, email, role, context) {
