...
import type { interviewDifficulties, interviewLevels } from "../models/interviewSession.model.js";
...
const roundNames: Record<InterviewLevel, string> = {
...
}): Promise<{ scores: AnswerScores; feedback: AnswerFeedback }> {
...
export function aggregateScores(scores: AnswerScores[]): AnswerScores {
  if (scores.length === 0) {
...
export function nextDifficulty(current: InterviewDifficulty, latestOverall: number): InterviewDifficulty {
  if (latestOverall >= 85) return current === "easy" ? "medium" : "hard";
  if (latestOverall < 55) return current === "hard" ? "medium" : "easy";
  return current;
}
...
// Fallback logic
function generateMockQuestion(input: any, anchor: string): GeneratedQuestion {
  return {
    level: input.level,
    roundName: roundNames[input.level as InterviewLevel],
...
function evaluateMockAnswer(input: any): { scores: AnswerScores; feedback: AnswerFeedback } {
