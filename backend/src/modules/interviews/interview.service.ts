...
import {
  aggregateScores,
  evaluateAnswer,
  generateQuestion as generateQuestionWithAi,
  nextDifficulty,
  type AnswerScores,
...
type QuestionSubdocument = InterviewSessionDocument["questions"][number];
...
export async function listInterviewSessions(userId: string, page: number, limit: number, status?: InterviewStatus) {
  const filter = {
    userId,
    isDeleted: false,
    ...(status ? { status } : {})
...
export async function generateQuestion(userId: string, sessionId: string, input: GenerateQuestionInput) {
...
function findQuestion(session: InterviewSessionDocument, questionId: string): QuestionSubdocument {
...
function toGeneratedQuestion(question: QuestionSubdocument): GeneratedQuestion {
  return {
...
function bestOverall(question: QuestionSubdocument): number {
  return question.attempts.reduce((best, attempt) => Math.max(best, attempt.scores.overall), 0);
}

function allLatestScores(session: InterviewSessionDocument): AnswerScores[] {
  return session.questions
    .map((question) => question.attempts[question.attempts.length - 1]?.scores)
    .filter((score): score is AnswerScores => Boolean(score));
}

function nextStepsFor(scores: AnswerScores): string[] {
  const steps: string[] = [];
